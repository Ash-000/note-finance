...
import {
  ArrowDown, ArrowUp, Bank, Bell, Briefcase, CalendarBlank, Camera,
  ...
  Sun, Trash, TrendDown, TrendUp, UserCircle, Wallet, X,
...
const seedTransactions = [
  { id: 't1', type: 'income', title: 'Gaji bulanan', category: 'Gaji', amount: 12000000, date: today, note: 'Pendapatan utama' },
  { id: 't2', type: 'expense', title: 'Belanja bulanan', category: 'Belanja', amount: 765000, date: today, note: 'Kebutuhan rumah' },
  { id: 't3', type: 'expense', title: 'Kopi sore', category: 'Makan & Minum', amount: 38000, date: today, note: '' },
  { id: 't4', type: 'income', title: 'Proyek freelance', category: 'Freelance', amount: 2500000, date: today, note: 'Landing page' },
  { id: 't5', type: 'expense', title: 'Transportasi', category: 'Transportasi', amount: 220000, date: today, note: 'KRL dan ojek' },
]
const seedGoals = [
  { id: 'g1', name: 'Kamera mirrorless', target: 12000000, saved: 7200000, deadline: '2026-12-20', icon: 'camera' },
  { id: 'g2', name: 'Liburan ke Jepang', target: 15000000, saved: 6000000, deadline: '2027-04-01', icon: 'gift' },
]
...
export default function App() {
  const [transactions, setTransactions] = useStoredState('arta-transactions', seedTransactions)
  const [goals, setGoals] = useStoredState('arta-goals', seedGoals)
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          <button className="nav-item"><Gear size={21} /><span>Pengaturan</span></button>
          <div className="account-card">
            <UserCircle size={38} weight="duotone" />
            <div><strong>Alex Pratama</strong><span>alex@email.com</span></div>
            <DotsThree size={22} />
          </div>
        </div>
      </aside>
      ...
      <main>
        <header className="topbar">
          <div><p className="eyebrow">{monthLabel.format(new Date())}</p><h1>{view === 'summary' ? 'Selamat datang, Alex' : title}</h1></div>
        ...
        {view === 'summary' && <Summary transactions={transactions} goals={goals} totals={totals} setView={setView} openModal={setModal} />}
        {(view === 'income' || view === 'expense') && <TransactionsPage type={view} transactions={transactions} total={view === 'income' ? totals.income : totals.expense} openModal={setModal} onDelete={deleteTransaction} />}
        {view === 'wishlist' && <WishlistPage goals={goals} openModal={setModal} onDelete={deleteGoal} />}
      </main>

      <nav className="mobile-nav" aria-label="Navigasi mobile">
        {[nav[0], { id: 'income', label: 'Transaksi', icon: Receipt }, nav[3]].map(({ id, label, icon: Icon }) => (
    ...
    <div className="dashboard-grid">
      <section className="balance-card">
        <div className="balance-main">
          <div className="section-icon"><Wallet size={22} weight="duotone" /></div><p>Sisa uang bulan ini</p><strong>{rupiah.format(totals.balance)}</strong>
          <div className="budget-line"><span style={{ width: `${spentPercent}%` }} /></div><small>{spentPercent}% pemasukan sudah digunakan</small>
        </div>
        <div className="totals-column">
          <div><span className="semantic income"><TrendUp size={20} /></span><p>Pemasukan</p><strong>{rupiah.format(totals.income)}</strong></div>
          <div><span className="semantic expense"><TrendDown size={20} /></span><p>Pengeluaran</p><strong>{rupiah.format(totals.expense)}</strong></div>
        </div>
        <div className="chart-column">
          <div className="chart-head"><span><ChartDonut size={20} />Pengeluaran</span><button onClick={() => setView('expense')}>Detail</button></div>
          {categories.length ? <><div className="donut" style={{ '--angle': `${Math.min((categories[0][1] / totals.expense) * 360, 360)}deg` }}><span>{categories.length}</span></div><div className="legend">{categories.map(([name, value]) => <div key={name}><span>{name}</span><strong>{rupiah.format(value)}</strong></div>)}</div></> : <EmptyCompact text="Belum ada pengeluaran" />}
        </div>
      </section>
